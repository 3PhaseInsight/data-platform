...
CREATE TABLE IF NOT EXISTS lv.secondary_substation (
    substation_id BIGINT PRIMARY KEY,
    zip_code      integer
);

CREATE TABLE IF NOT EXISTS lv.transformer (
    transformer_id BIGINT PRIMARY KEY,
    substation_id  BIGINT NOT NULL REFERENCES lv.secondary_substation(substation_id) ON DELETE CASCADE,
    capacity_kva   NUMERIC
);
CREATE INDEX IF NOT EXISTS transformer_substation_idx ON lv.transformer (substation_id);

CREATE TABLE IF NOT EXISTS lv.lv_feeder (
    feeder_id      BIGINT PRIMARY KEY,
    transformer_id BIGINT NOT NULL REFERENCES lv.transformer(transformer_id) ON DELETE CASCADE,
    fuse_size_amps integer
);
CREATE INDEX IF NOT EXISTS lv_feeder_transformer_idx ON lv.lv_feeder (transformer_id);
...
CREATE TABLE IF NOT EXISTS lv.delivery_point (
    ...
    cabinet_id               BIGINT NOT NULL REFERENCES lv.cabinet(cabinet_id) ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS lv.meter (
    ...
    delivery_point_id BIGINT NOT NULL REFERENCES lv.delivery_point(delivery_point_id) ON DELETE RESTRICT,
...
-- Nodes (versioned)
CREATE TABLE IF NOT EXISTS lv.lv_node (
    version    integer      NOT NULL,      -- snapshot number
    node_id    BIGSERIAL    NOT NULL,
    node_type  lv.node_type NOT NULL,
    feeder_id  BIGINT REFERENCES lv.lv_feeder(feeder_id) ON DELETE CASCADE,
    cabinet_id BIGINT REFERENCES lv.cabinet(cabinet_id)  ON DELETE CASCADE,
...
CREATE INDEX IF NOT EXISTS lv_node_by_type_idx    ON lv.lv_node (version, node_type);
CREATE INDEX IF NOT EXISTS lv_node_feeder_fk_idx  ON lv.lv_node (version, feeder_id);
CREATE INDEX IF NOT EXISTS lv_node_cabinet_fk_idx ON lv.lv_node (version, cabinet_id);

-- Logical edges (versioned) – use edge_id (not cable_id) for clarity
CREATE TABLE IF NOT EXISTS lv.lv_edge (
    ...
    PRIMARY KEY (version, edge_id),
    CHECK (node1_id <> node2_id),

    -- Endpoints must exist in the SAME version
    FOREIGN KEY (version, node1_id) REFERENCES lv.lv_node (version, node_id) ON DELETE CASCADE,
    FOREIGN KEY (version, node2_id) REFERENCES lv.lv_node (version, node_id) ON DELETE CASCADE
);
...
CREATE UNIQUE INDEX IF NOT EXISTS lv_edge_undirected_uniq
    ON lv.lv_edge (version, LEAST(node1_id, node2_id), GREATEST(node1_id, node2_id));

CREATE INDEX IF NOT EXISTS lv_edge_node1_ver_idx ON lv.lv_edge (version, node1_id);
CREATE INDEX IF NOT EXISTS lv_edge_node2_ver_idx ON lv.lv_edge (version, node2_id);
...
CREATE TABLE IF NOT EXISTS lv.lv_edge_cable (
    version  integer NOT NULL,
    ...
    FOREIGN KEY (version, edge_id)  REFERENCES lv.lv_edge  (version, edge_id)  ON DELETE CASCADE,
    FOREIGN KEY (version, cable_id) REFERENCES lv.cable    (version, cable_id) ON DELETE CASCADE,

    UNIQUE (version, edge_id, seq_no)
);
CREATE INDEX IF NOT EXISTS lv_edge_cable_edge_idx  ON lv.lv_edge_cable (version, edge_id);
CREATE INDEX IF NOT EXISTS lv_edge_cable_cable_idx ON lv.lv_edge_cable (version, cable_id);

-- =========================
-- Convenience views
-- =========================
CREATE OR REPLACE VIEW lv.lv_node_current AS
SELECT n.* FROM lv.lv_node n JOIN lv.topology_version v ON v.version=n.version WHERE v.is_current;

CREATE OR REPLACE VIEW lv.lv_edge_current AS
SELECT e.* FROM lv.lv_edge e JOIN lv.topology_version v ON v.version=e.version WHERE v.is_current;

CREATE OR REPLACE VIEW lv.lv_edge_current_with_totals AS
SELECT e.version, e.edge_id, e.node1_id, e.node2_id,
...
FROM lv.lv_edge e
         ...
         JOIN lv.lv_edge_cable ec ON ec.version=e.version AND ec.edge_id=e.edge_id
         JOIN lv.cable c ON c.version=ec.version AND c.cable_id=ec.cable_id
GROUP BY e.version, e.edge_id, e.node1_id, e.node2_id;
